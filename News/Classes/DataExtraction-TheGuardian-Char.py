...
class DataExtraction:
    ...
    def NewsExtraction(self, response):
        ...
        for article in response_json:
            ...
            df.to_csv(str(os.getcwd()) + "/News/Classes/" + config["q"] + "_" + str(self.page) + ".csv", index=False) 
            print(df['Title'])
...
if concat_csv.upper() =="YES":
    ...
    for i in range(1,(data[2] + 1)):
        files.append(str(os.getcwd()) + "/News/Classes/" + config["q"] +"_" + str(i) + ".csv")

    for filename in files:
        df = pd.read_csv(filename, index_col=None, header=0)
        li.append(df)
    combined = pd.concat(li, axis=0, ignore_index=True)
    combined.to_csv(str(os.getcwd()) + "/News/Classes/" + config["q"] + ".csv", index=False)
